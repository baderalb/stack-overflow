...
.elementCard {
    width: 900px; /* Maintain card width */
    ...
    <hr><hr><hr><hr><hr><hr>
       ...
       <?php
        ...
        ?>
         ...
         <div class="containerHeader2"style="margin-right:200px;margin-bottom: 20px;" >
        <h2 style="margin-right:500px;"> Sorted By Most Answers</h2>
        <div>
            <a href="all_questions.php" class="btn btn-outline-primary  " style="margin-right:170px;font-size:17px">Sort By: Recency</a>
            </div></div>

        <?php
        $questions =getTopAnsweredQuestions();;
        foreach ($questions as $question) {
            $s1= '<form method="post" action="#">
                <div class="card elementCard border-left-0 " style="margin-left:490px ">
                    <div class="card-body">
                    <div class="containerQustionContent">
                    <h5> '.substr($question['title'], 0, 25) . (strlen($question['title']) > 30 ? '...' : '').' </h5> <a class="card-link btn" style="font-size: 18px; text-decoration:none;" href="index.php?question_id=' . $question['id'] . '"> View More</a>
                      </div> 
                       <hr>
                    <div class="card-subtitle text-muted  containerQustionContent">
                        <div>
                            <h6 class="mt-1" style="display: inline;"> ' . substr(htmlspecialchars_decode($question['description']) , 0,60) . (strlen($question['description']) > 69 ? '...' : '')   . '</h6>
                            <br>';
                            if (isLoggedIn()) {
